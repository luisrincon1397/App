...
import NotasModel from '../components/NotasModel';
...
export default function NotasScreen({navigation}) {
  ...
  useEffect(() => {
    ...
    fetch('http://dtai.uteq.edu.mx/~alalui186/awi4/ws/integradora/notas')
      .then(response => response.json())
      .then(respJson => {
        let var1 = respJson["notas"];
        setNotas(var1);
        setRed(true);
      })
    ...
    if(nota !== ''){
      const data = new FormData();
            data.append('nota', nota);
                fetch('http://dtai.uteq.edu.mx/~alalui186/awi4/ws/integradora/agregar_nota', {
                  method: 'POST',
                  body: data
          ...
          {nvlUsuario == 1 ? (
            <View style={styles.newImgDiv}>
                <TouchableOpacity style={styles.tO} onPress={()=>{setVisible(true)}}>
                    <Text style={{fontSize: 30,color: '#4D4D4D'}}>Agregar nota </Text>
            ...
            <Modal animationType={"slide"} transparent={true} visible={visible}>
              <View style={styles.modalStyle}>
                <Text style={styles.titulo}>Ingresa la nota</Text>
                <TextInput style={styles.texto} keyboardType={"default"} placeholder='Ingrese su nota aqui...' onChangeText={(text) => setNewNota(text)} value={newNota}/> 
                <View style={styles.viewHori}>
                  ...
                  <TouchableOpacity style={styles.btnCancel} onPress={()=>{guardarNota(newNota),setNewNota('')}}>
            ...
            <View style={styles.codeHighlightContainer}>
                ...
                keyExtractor={datos => datos.idnota}
            />
            </View>
          </View>
          ) : (
